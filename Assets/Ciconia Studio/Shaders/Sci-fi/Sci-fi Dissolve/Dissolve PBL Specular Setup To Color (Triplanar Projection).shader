Shader "Ciconia Studio/Effects/Sci-Fi/Dissolve/Specular Setup/Dissolve To Color (Triplanar Projection)" {
    Properties {
        ...
        [Space(15)]_SpecColor2 ("Specular Color", Color) = (1,1,1,1)
...
///////// Gloss:
                float4 _SpecGlossMap1 = tex2D(_SpecGlossMap,TRANSFORM_TEX(i.uv0, _SpecGlossMap));
                float4 _SpecularmapGlossA2 = tex2D(_SpecGlossMap,TRANSFORM_TEX(i.uv0, _SpecGlossMap));
                float3 NormalDirectionmask = mul( unity_WorldToObject, float4(saturate(((abs(i.normalDir)*2.0)*0.5)),0) ).xyz.rgb; // NormalDirectionMask
                float3 node_3553 = NormalDirectionmask;
                float3 node_3518 = mul( unity_WorldToObject, float4((i.posWorld.rgb-objPos.rgb),0) ).xyz;
                float2 GB = node_3518.rgb.gb;
                float2 node_3448 = GB;
                float4 _node_1257 = tex2D(_TexAssetMAsk,TRANSFORM_TEX(node_3448, _TexAssetMAsk)); // X Axis FrontBack
                float2 RB = node_3518.rgb.rb;
                float2 node_2818 = RB;
                float4 _node_4330 = tex2D(_TexAssetMAsk,TRANSFORM_TEX(node_2818, _TexAssetMAsk)); // Y Axis TopBottom
                float2 RG = node_3518.rgb.rg;
                float2 node_7719 = RG;
                float4 _node_3650 = tex2D(_TexAssetMAsk,TRANSFORM_TEX(node_7719, _TexAssetMAsk)); // Z Axis LeftRight 
                float node_2467 = (node_3553.r*(_node_1257.r*NormalDirectionmask.r) + node_3553.g*(_node_4330.r*NormalDirectionmask.g) + node_3553.b*(_node_3650.r*NormalDirectionmask.b));
                float node_7714 = 0.0;
                float node_922 = (1.0+(-1*_Contrast));
                float node_370 = saturate(((_MaskAmount*2.0+-1.0)+(node_922 + ( (lerp( node_2467, (1.0 - node_2467), _Invertmask ) - node_7714) * (_Contrast - node_922) ) / (1.0 - node_7714))));
                float Mask = node_370;
                float Glossiness = lerp((_SpecGlossMap1.a*_Glossiness),(_SpecularmapGlossA2.a*_Glossiness2),Mask);
...
////// Specular:
                ...
                float3 Specularmap = lerp((_SpecColor.rgb*_SpecGlossMap1.rgb*_SpecularIntensity),(_SpecColor2.rgb*_SpecularmapGlossA2.rgb*_SpecularIntensity2),Mask);
                float3 specularColor = Specularmap;
                float specularMonochrome;
                float4 _MainTex1 = tex2D(_MainTex,TRANSFORM_TEX(i.uv0, _MainTex));
                float4 _DetailAlbedoMap = tex2D(_MainTex,TRANSFORM_TEX(i.uv0, _MainTex));
                float node_6047 = Mask;
                float3 Diffusemap = lerp((_Color.rgb*lerp(_MainTex1.rgb,dot(_MainTex1.rgb,float3(0.3,0.59,0.11)),_DesaturateAlbedo)),(_Color2.rgb*lerp(_DetailAlbedoMap.rgb,dot(_DetailAlbedoMap.rgb,float3(0.3,0.59,0.11)),_DesaturateAlbedo2)),node_6047);
