Shader "Ciconia Studio/Effects/Sci-Fi/Dissolve/Specular Setup/Dissolve To Transparent (Triplanar Projection)" {
    Properties {
    ...
    SubShader {
        ...
        Pass {
            ...
            float4 frag(VertexOutput i, float facing : VFACE) : COLOR {
                ...
                float3 node_5548 = mul( unity_WorldToObject, float4((i.posWorld.rgb-objPos.rgb),0) ).xyz;
                float2 GB = node_5548.rgb.gb;
                float2 node_6022 = GB;
                float4 _node_1257 = tex2D(_TexAssetMAsk,TRANSFORM_TEX(node_6022, _TexAssetMAsk)); // X Axis FrontBack
                float2 RB = node_5548.rgb.rb;
                float2 node_5945 = RB;
                float4 _node_4330 = tex2D(_TexAssetMAsk,TRANSFORM_TEX(node_5945, _TexAssetMAsk)); // Y Axis TopBottom
                float2 RG = node_5548.rgb.rg;
                float2 node_1270 = RG;
                float4 _node_3650 = tex2D(_TexAssetMAsk,TRANSFORM_TEX(node_1270, _TexAssetMAsk)); // Z Axis LeftRight 
                float node_4567 = (node_3867.r*(_node_1257.r*NormalDirectionMask.r) + node_3867.g*(_node_4330.r*NormalDirectionMask.g) + node_3867.b*(_node_3650.r*NormalDirectionMask.b));
                float node_3462 = 0.0;
                float node_6202 = (1.0+(-1*_Contrast));
                float node_8230 = saturate(((_MaskAmount*2.0+-1.0)+(node_6202 + ( (lerp( node_4567, (1.0 - node_4567), _Invertmask ) - node_3462) * (_Contrast - node_6202) ) / (1.0 - node_3462))));
                float Mask = node_8230;
